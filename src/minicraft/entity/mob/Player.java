...
public class Player extends Mob implements ItemHolder, ClientTickable {
	...
	public static MobSprite[][] sprites =  MobSprite.compileMobSpriteAnimations(0, 0,4);
	private static MobSprite[][] carrySprites = MobSprite.compileMobSpriteAnimations(0, 2,4); // the sprites while carrying something.
	private static MobSprite[][] suitSprites = MobSprite.compileMobSpriteAnimations(8, 0,4); // the "airwizard suit" sprites.
	private static MobSprite[][] carrySuitSprites = MobSprite.compileMobSpriteAnimations(8, 2, 4); // the "airwizard suit" sprites.
	...
	public void render(Screen screen) {
		...
		if (isSwimming()) {
			...
			if (level.getTile(x / 16, y / 16) == Tiles.get("water")) {
				screen.render(xo + 0, yo + 3, 5 + 2 * 32, 0, 4); // render the water graphic
				screen.render(xo + 8, yo + 3, 5 + 2 * 32, 1, 4); // render the mirrored water graphic to the right.
			} else if (level.getTile(x / 16, y / 16) == Tiles.get("lava")) {
				screen.render(xo + 0, yo + 3, 6 + 2 * 32, 1, 4); // render the water graphic
				screen.render(xo + 8, yo + 3, 6 + 2 * 32, 0, 4); // render the mirrored water graphic to the right.
			}
		}
		...
		if (activeItem instanceof TileItem) {
			...
			screen.render(t.x * 16 + 4, t.y * 16 + 4, 3 + 4 * 32, 0, 4);
		}

		if (attackTime > 0 && attackDir == Direction.UP) { // if currently attacking upwards...
			screen.render(xo + 0, yo - 4, 3 + 2 * 32, 0, 4); //render left half-slash
			screen.render(xo + 8, yo - 4, 3 + 2 * 32, 1, 4); //render right half-slash (mirror of left).
